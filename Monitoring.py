...
# list of UAV Img
MPath = 'image_CP/' # UAV Image Folder
ExportImgPath = 'result_CP/' # Save Folder
ReportPath = 'report_CP/'
...
for Img_i, Img in enumerate(ListImg):
	...
	# Report
	if Img_i % 20 == 0:
		ReportName = ReportPath + 'log' + '/' + 'Monitoring_report_' + str(Img_i) + '.csv'
...
np.savetxt(ReportPath + 'Monitoring_report.csv', 
			DataTable, 
			delimiter=',', 
			fmt='%3.5f',
			header='lat,lon,n_styrofoam,n_PET',
